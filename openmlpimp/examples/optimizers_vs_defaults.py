...
def plot_boxplot(priors_results, mode):
    ...
    for idx, classifier in enumerate(priors_results.keys()):
        ...
        for task_id in priors_results[classifier]:
            if len(priors_results[classifier][task_id]) == 2 and 'uniform' in priors_results[classifier][task_id] and 'kde' in priors_results[classifier][task_id]:
                current = priors_results[classifier][task_id]['kde'] - priors_results[classifier][task_id]['uniform']
                data.append(current)
        ...
        plt.violinplot(data)
        plt.savefig('/home/vanrijn/experiments/priors_%s_%s.pdf' %(classifier, mode), bbox_inches='tight')
        plt.close()
    ...
    plt.savefig('/home/vanrijn/experiments/priors_violin_%s.pdf' %mode, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    priors_results_test = collections.defaultdict(lambda: collections.defaultdict(dict))
    priors_results_valid = collections.defaultdict(lambda: collections.defaultdict(dict))
    ...
    for classifier, directory_suffix in all_classifiers:
        for task_id in all_tasks:
            ...
            for strategy in os.listdir(args.result_directory + directory_suffix):
                strategy_name = strategy.split('__')[0]
                strategy_trace = args.result_directory + directory_suffix + '/' + strategy + '/' + str(task_id) + '/trace.arff'
                strategy_predictions = args.result_directory + directory_suffix + '/' + strategy + '/' + str(task_id) + '/predictions.arff'

                if not os.path.isfile(strategy_predictions) or not os.path.isfile(strategy_trace):
                    print(openmlpimp.utils.get_time(), '%s: Task %d not finished for strategy %s' %(classifier, task_id, strategy))
                    continue
                ...
                priors_results_test[classifier][task_id][strategy_name] = score.mean()
                priors_results_valid[classifier][task_id][strategy_name] = trace_to_score(strategy_trace)
