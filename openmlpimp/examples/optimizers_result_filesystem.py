...
def parse_args():
    ...
    parser.add_argument('--setup', type=str, default='hyperband_5', help='the bandit that was ran')
...
if __name__ == '__main__':
    ...
    result_directory = args.result_directory + '/' + args.setup + '/' + str(args.flow_id) + '/' + results_suffix
    output_directory = args.output_directory + '/' + args.setup + '/' + str(args.flow_id) + '/' + results_suffix
    ...
    for strategy in all_strategies:
        ...
        for task in task_directories:
            ...
            for seed in os.listdir(os.path.join(directory, task)):

                arff_file = os.path.join(result_directory, strategy, task, seed, 'trace.arff')
                if os.path.isfile(arff_file):
                    ...
                    # all_traces[task][strategy][seed] = trace

                    output_indivudual = output_directory + '/curves/' + strategy + '/' + task + '/' + seed
                    output_averaged = output_directory + '/curves_avg/' + strategy + '/' + task
                    openmlpimp.utils.obtain_performance_curves(trace, output_indivudual, output_averaged, seed)
